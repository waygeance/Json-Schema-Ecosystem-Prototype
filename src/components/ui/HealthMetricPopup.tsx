...
interface HealthMetricPopupProps {
  isOpen: boolean;
  onClose: () => void;
  metric: HealthMetricInfo;
}

export function HealthMetricPopup({ isOpen, onClose, metric }: HealthMetricPopupProps) {
...
interface HealthMetricInfoButtonProps {
  metric: HealthMetricInfo;
  className?: string;
}

export function HealthMetricInfoButton({ metric, className }: HealthMetricInfoButtonProps) {
  ...
  return (
    <>
      ...
      <HealthMetricPopup isOpen={isPopupOpen} onClose={() => setIsPopupOpen(false)} metric={metric} />
    </>
  );
}
